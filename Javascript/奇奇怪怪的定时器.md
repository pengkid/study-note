...
#### <font color="4590a3">"this"的问题{#index1_13}</font>
...
#### <font color="4590a3">语法{#index1_21}</font>
...
// 先输出1，再输出0
```
因为即使setTimeout规定了延迟零秒调用，但定时器一旦执行，就会触发定时器线程，并在指定时间结束后将匿名函数放置在JS引擎线程的队列之后，并且计划在下一个机会运行，而不是立即运行。目前正在执行的代码必须在执行队列后面的功能之前完成，因此是先输出1，再输出0。（简单的程序执行也就 1ms ）
