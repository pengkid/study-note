...
2. 如果当前节点不为空节点，深度加一，并且执行如下递归：

    a. 递归遍历当前节点的左子树，直到叶子结点

    b. 递归遍历当前结点的右子树，直到叶子结点

    c. 比较遍历左右子树得到的深度，返回较大值
...
算发思想：首先将根节点推入数组。当数组不为空的时候，执行循环：取出数组的一个节点，如果该结点的左子树为非空，则将该结点的左子树入数组；如果该结点的右子树为非空，则将该结点的右子树入数组。 
